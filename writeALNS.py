...
def toCsv(alns, outputPath="./result/", outputName=None, reset=False):
    ...
    if not os.path.isfile(fileName) or reset:
        with open(fileName, "w") as outfile:
            line = "Name; Evolution_cost; Evolution_iter_best; Evolution_time_best; Cost_best; Iter_best; Time_best; Number of iterations"
            for method in alns.USED_METHODS:
                line += "; " + method
            outfile.write(line + "\n")
    with open(fileName, "a") as outfile:
        line = "{name}; {evolution_cost}; {evolution_iter_best}; {evolution_time_best}; {cost_best}; {iter_best}; {time_best}; {nIter}".format(
            name=alns.instance.getName(),
